...
const WishlistPage = () => {
      ...
      if (wishlistResponse.data.success) {
        ...
        const games = gameResponses
          ...
          .map(response => ({
            ...
            addedAt: items.find(item => item.gameId === response.data.data.steamAppId)?.addedAt
          }));
  ...
  const handleRemoveFromWishlist = async (gameId) => {
    try {
      ...
      if (response.data.success) {
        // Update state
        setWishlistItems(prev => prev.filter(item => item.gameId !== gameId));
        setGameDetails(prev => prev.filter(game => game.steamAppId !== gameId));
  ...
  const sortedGames = [...gameDetails].sort((a, b) => {
    const getPrice = (game) => {
      const price = typeof game.price === 'object' ? game.price.amount : game.price;
    ...
    return (
      <div className="wishlist-page">
        <div className="wishlist-empty">
          ...
          <div className="empty-actions">
            <Link to="/browse" className="browse-btn">Browse Games</Link>
            <Link to="/" className="home-btn">Go to Homepage</Link>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`wishlist-game-card ${isRemoving ? 'removing' : ''}`}>
      ...
      <div className="wishlist-actions-card">
        <button className="action-btn-card primary" onClick={handleAddToCartClick}>
          Add to Cart
        </button>
